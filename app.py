...
with tab4:
    ...
    if not df_chart.empty:
        # Create a stunning Altair chart with linear gradient and rounded edges
        chart = alt.Chart(df_chart).mark_bar(
            ...
            color=alt.Gradient(
                ...
                stops=[alt.GradientStop(color='#6366f1', offset=0),
                       alt.GradientStop(color='#a855f7', offset=1)],
                x1=0,
                y1=0,
                x2=1,
                y2=0
            ...
            x=alt.X('count:Q', title='Seçilen Kitap Sayısı', axis=alt.Axis(grid=False, tickMinStep=1)),
            y=alt.Y('suggested_by:N', title='Üye', sort='-x'),
            tooltip=[
                alt.Tooltip('suggested_by:N', title='Üye'),
                alt.Tooltip('count:Q', title='Seçilen Kitap Sayısı')
            ]
        ).properties(
            height=300
        ).configure_axis(
